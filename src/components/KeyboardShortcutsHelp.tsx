    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 max-h-[70vh] overflow-y-auto space-y-6">
          {KEYBOARD_SHORTCUT_SECTIONS.map((section) => (
            <div key={section.title}>
              <h3 className="text-label font-bold uppercase tracking-wider text-white/50 mb-3">{section.title}</h3>
                    ...
                    <div className="flex items-center gap-1.5">
                      {shortcut.keys.map((key, i) => (
                        <span key={i} className="flex items-center gap-1.5">
                          {i > 0 && <span className="text-meta text-white/20">or</span>}
        ...
        <div className="px-6 py-3 border-t border-white/[0.06] flex justify-center">
          <span className="text-label text-white/25">Press <kbd className="px-1.5 py-0.5 text-meta font-bold bg-white/[0.06] border border-white/[0.06] rounded">?</kbd> or <kbd className="px-1.5 py-0.5 text-meta font-bold bg-white/[0.06] border border-white/[0.06] rounded">Esc</kbd> to close</span>
        </div>
      </div>
    </div>,
    document.body,
