...
function DebugRow({ label, value }: { label: string; value: unknown }) {
  return (
    <div className="grid grid-cols-[8.5rem_minmax(0,1fr)] gap-3 border-b border-white/6 py-2 last:border-0">
      ...
      <dd className="min-w-0 break-all font-mono text-[11px] text-white/75">{valueText(value)}</dd>
    </div>
  )
}
      ...
      <div className="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="rounded bg-amber-400 px-1.5 py-0.5 text-[9px] font-black tracking-wider text-black">DEV</span>
            <h3 className="text-sm font-bold text-white">Player format diagnostics</h3>
          </div>
          <p className="mt-1 text-[11px] text-white/40">Live mpv values. “Detected” means the active stream signals that format.</p>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={copySnapshot} disabled={!snapshot} className="rounded-lg border border-white/10 px-2.5 py-1.5 text-[10px] font-semibold text-white/55 hover:bg-white/8 hover:text-white disabled:opacity-30">{copied ? 'Copied' : 'Copy JSON'}</button>
          <button type="button" onClick={onRefresh} disabled={loading} className="rounded-lg border border-white/10 px-2.5 py-1.5 text-[10px] font-semibold text-white/55 hover:bg-white/8 hover:text-white disabled:opacity-30">{loading ? 'Reading…' : 'Refresh'}</button>
          <button type="button" onClick={onClose} aria-label="Close player diagnostics" className="grid h-7 w-7 place-items-center rounded-full text-lg text-white/45 hover:bg-white/10 hover:text-white">×</button>
        </div>
      </div>
      ...
      <div className="overflow-y-auto p-5">
        ...
        <section>
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className={`rounded-full px-2 py-1 text-[9px] font-bold ${encodedOutput ? 'bg-emerald-400/15 text-emerald-300' : passthroughConfigured ? 'bg-amber-400/15 text-amber-300' : 'bg-white/7 text-white/40'}`}>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {formats.map((format) => (
              <div key={format.id} title={format.detail} className={`rounded-xl border px-3 py-2 ${format.state === 'detected' ? 'border-emerald-400/25 bg-emerald-400/10' : format.state === 'carrier' ? 'border-amber-400/20 bg-amber-400/8' : 'border-white/7 bg-white/[0.025]'}`}>
                <p className={`text-[11px] font-semibold ${format.state === 'detected' ? 'text-emerald-200' : format.state === 'carrier' ? 'text-amber-200' : 'text-white/38'}`}>{format.label}</p>
                <p className="mt-0.5 text-[9px] uppercase tracking-wider text-white/30">{format.state === 'detected' ? 'Detected' : format.state === 'carrier' ? 'Carrier only' : 'Not detected'}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="mt-5 grid gap-x-6 md:grid-cols-2">
          <dl>
            <h4 className="mb-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white/45">Audio path</h4>
          ...
          <dl className="mt-5 md:mt-0">
            <h4 className="mb-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white/45">Video path</h4>
        ...
        {snapshot && <p className="mt-4 text-right font-mono text-[9px] text-white/25">Updated {new Date(snapshot.capturedAt).toLocaleTimeString()}</p>}
      </div>
    </div>
  )
}
