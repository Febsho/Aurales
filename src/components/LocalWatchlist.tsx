    ...
    <section aria-labelledby="local-watchlist-heading">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-white/60">{items.length} {items.length === 1 ? 'title' : 'titles'} saved on this device</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {([['all', 'All'], ['movie', 'Movies'], ['series', 'Shows']] as const).map(([value, label]) => (
              <button
                ...
                className={`rounded-lg border px-3 py-1.5 text-label font-semibold transition-colors cursor-pointer ${typeFilter === value ? 'border-accent/25 bg-accent/15 text-accent' : 'border-white/[0.07] bg-white/[0.03] text-white/60 hover:bg-white/[0.06] hover:text-white/70'}`}
              >
                {label}
              </button>
            ))}
            <div className="relative ml-1">
              ...
              <input
                ...
                className="w-44 rounded-lg border border-white/[0.07] bg-white/[0.04] py-1.5 pl-8 pr-3 text-label text-white placeholder-white/25 outline-none transition-colors focus:border-white/[0.16] focus:bg-white/[0.07]"
              />
            </div>
          </div>
        )}
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.07] bg-white/[0.025] px-6 py-20 text-center">
          ...
          <p className="text-sm font-medium text-white/60">Your local watchlist is empty</p>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] py-16 text-center">
          <p className="text-sm font-medium text-white/60">No watchlist titles match this filter</p>
        </div>
      ) : (
