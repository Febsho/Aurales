  ...
  return (
    <>
      <div className="mb-5 ml-3 inline-flex rounded-xl border border-white/[0.07] bg-white/[0.03] p-1">
        <button onClick={() => setActivityView('local')} className={`rounded-lg px-4 py-2 text-xs font-bold transition-all ${activityView === 'local' ? 'bg-accent/15 text-accent' : 'text-white/60 hover:text-white/65'}`}>Local Activity</button>
        <button onClick={() => setActivityView('connected')} className={`rounded-lg px-4 py-2 text-xs font-bold transition-all ${activityView === 'connected' ? 'bg-accent/15 text-accent' : 'text-white/60 hover:text-white/65'}`}>Connected History</button>
      </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-white/[0.07] bg-gradient-to-br from-white/[0.05] to-accent/[0.04] p-5 sm:p-6">
        <div><h2 className="text-lg font-bold text-white">Viewing Activity</h2><p className="mt-1 text-xs text-white/60">Private statistics from playback inside Aurales.</p></div>
        <div className="flex flex-wrap gap-2">
          <div className="flex rounded-xl border border-white/[0.08] bg-black/20 p-1">{(['7d', '30d', '12m', 'all'] as Range[]).map((value) => <button key={value} onClick={() => setRange(value)} className={`rounded-lg px-3 py-1.5 text-meta font-bold uppercase ${range === value ? 'bg-accent/15 text-accent' : 'text-white/60 hover:text-white/65'}`}>{value}</button>)}</div>
          <div className="flex rounded-xl border border-white/[0.08] bg-black/20 p-1">{(['all', 'movie', 'series', 'anime'] as Media[]).map((value) => <button key={value} onClick={() => setMedia(value)} className={`rounded-lg px-3 py-1.5 text-meta font-bold capitalize ${media === value ? 'bg-accent/15 text-accent' : 'text-white/60 hover:text-white/65'}`}>{value}</button>)}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        ].map(([label, value]) => <div key={label} className="rounded-2xl border border-white/[0.07] bg-white/[0.035] p-4"><p className="text-meta font-bold uppercase tracking-wider text-white/50">{label}</p><p className="mt-2 text-xl font-black text-white/85">{value}</p></div>)}
      </div>

      <div className="grid gap-4 xl:grid-cols-[1.7fr_1fr]">
        <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5">
          <div className="mb-5 flex items-center justify-between"><div><h3 className="text-sm font-bold text-white/80">Watch time</h3><p className="mt-1 text-meta text-white/50">Minutes watched by active day</p></div><span className="text-xs font-bold text-accent/75">{durationLabel(summary.totalSeconds)}</span></div>
