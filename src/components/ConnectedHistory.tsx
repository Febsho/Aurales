...
export default function ConnectedHistory() {
  ...
  return <section className="space-y-4">
    <div className="rounded-2xl border border-white/[0.07] bg-gradient-to-br from-white/[0.05] to-accent/[0.035] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-bold text-white">Connected History</h2><p className="mt-1 text-xs text-white/60">Combine watched history from selected services. Matching titles are shown once.</p></div><button onClick={() => setRefreshToken((value) => value + 1)} disabled={loading || !selected.length} className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2 text-xs font-bold text-white/60 hover:bg-white/[0.08] disabled:opacity-30">{loading ? 'Refreshing...' : 'Refresh history'}</button></div>
      <div className="mt-5 flex flex-wrap gap-2">{connected.map((source) => <button key={source} onClick={() => toggleSource(source)} className={`rounded-xl border px-3 py-2 text-xs font-bold transition-all ${selected.includes(source) ? sourceColors[source] : 'border-white/[0.06] bg-white/[0.025] text-white/50 hover:text-white/60'}`}>{sourceLabels[source]}{counts[source] != null ? ` · ${counts[source]}` : ''}</button>)}{connected.length > 1 && <button onClick={() => setSelected(selected.length === connected.length ? [] : connected)} className="rounded-xl border border-white/[0.06] bg-white/[0.025] px-3 py-2 text-xs font-bold text-white/60 hover:text-white/65">{selected.length === connected.length ? 'Clear all' : 'Use all'}</button>}</div>
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-7">{[
      ...
      return <button type="button" key={String(label)} disabled={!isWatchTime || !estimatedMinutes} onClick={isWatchTime ? () => setWatchTimeInDays((current) => !current) : undefined} className={`rounded-2xl border border-white/[0.07] bg-white/[0.035] p-4 text-left ${isWatchTime && estimatedMinutes ? 'cursor-pointer transition-colors hover:border-accent/25 hover:bg-white/[0.055]' : ''}`}><p className="text-meta font-bold uppercase tracking-wider text-white/50">{label}</p><p className="mt-2 text-xl font-black text-white/85">{value}</p><p className="mt-1 text-tag text-white/22">{detail}</p></button>
    })}</div>

    <div className="grid gap-4 xl:grid-cols-[1.7fr_1fr]">
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5"><div className="mb-5 flex items-center justify-between"><div><h3 className="text-sm font-bold text-white/80">History timeline</h3><p className="mt-1 text-meta text-white/50">Unique titles by latest watched month</p></div><span className="text-xs font-bold text-accent/70">{statsItems.length} titles</span></div>{monthly.length ? <div className="flex h-40 items-end gap-2">{monthly.map(([month, count]) => <div key={month} className="group flex min-w-0 flex-1 flex-col items-center justify-end gap-2" title={`${month}: ${count} titles`}><div className="w-full min-h-1 rounded-t bg-accent/35 transition-colors group-hover:bg-accent" style={{ height: `${Math.max(4, count / maxMonth * 120)}px` }} /><span className="hidden text-tag text-white/25 sm:block">{month.slice(5)}</span></div>)}</div> : <div className="grid h-40 place-items-center text-sm text-white/25">No dated history from the selected services.</div>}</div>
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5"><h3 className="text-sm font-bold text-white/80">Content mix</h3><div className="mt-5 space-y-4">{mediaCounts.map(({ type, count }) => <div key={type}><div className="mb-1.5 flex justify-between text-xs"><span className="capitalize text-white/60">{type}</span><span className="font-semibold text-white/60">{count} titles</span></div><div className="h-2 overflow-hidden rounded-full bg-white/[0.05]"><div className="h-full rounded-full bg-accent/65" style={{ width: `${count / maxMediaCount * 100}%` }} /></div></div>)}</div></div>
    </div>

    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5"><div className="mb-5 flex items-center justify-between"><div><h3 className="text-sm font-bold text-white/80">Favorite genres</h3><p className="mt-1 text-meta text-white/50">Estimated watch time from TMDB metadata</p></div>{runtimeProgress.total > 0 && runtimeProgress.completed < runtimeProgress.total && <span className="text-meta font-semibold text-accent/65">Analyzing {runtimeProgress.completed}/{runtimeProgress.total}</span>}</div>{genreTotals.length ? <div className="grid gap-x-8 gap-y-4 md:grid-cols-2">{genreTotals.map((item) => <div key={item.genre}><div className="mb-1.5 flex justify-between text-xs"><span className="text-white/60">{item.genre}</span><span className="text-white/50">{formatMinutes(item.minutes)}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/[0.05]"><div className="h-full rounded-full bg-accent/60" style={{ width: `${item.minutes / genreTotals[0].minutes * 100}%` }} /></div></div>)}</div> : <div className="py-8 text-center text-sm text-white/25">Genre insights appear as TMDB metadata is calculated.</div>}</div>

    <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex rounded-xl border border-white/[0.08] bg-black/20 p-1">{(['all', 'movie', 'series', 'anime'] as const).map((value) => <button key={value} onClick={() => setMedia(value)} className={`rounded-lg px-3 py-1.5 text-meta font-bold capitalize ${media === value ? 'bg-accent/15 text-accent' : 'text-white/60 hover:text-white/65'}`}>{value}</button>)}</div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search connected history..." className="w-64 rounded-xl border border-white/[0.08] bg-white/[0.04] px-3.5 py-2.5 text-xs text-white outline-none placeholder:text-white/25 focus:border-accent/30" /></div>

    {loading && items.length === 0 ? <div className="py-16 text-center text-sm text-white/50">Combining connected history...</div> : selected.length === 0 ? <div className="py-16 text-center text-sm text-white/50">Select at least one connected service.</div> : <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{visibleItems.map((item) => <button key={item.id} onClick={() => openItem(item)} className="group flex min-w-0 items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.03] p-3 text-left transition-all hover:border-accent/20 hover:bg-white/[0.055]">
      {item.poster ? <img src={item.poster} alt="" className="h-20 w-14 flex-none rounded-lg object-cover" loading="lazy" /> : <div className="grid h-20 w-14 flex-none animate-pulse place-items-center rounded-lg bg-white/[0.055] text-tag text-white/20">{enrichingPosters ? 'Loading' : 'No poster'}</div>}
      <span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-white/80">{item.title}</span><span className="mt-1 block text-meta capitalize text-white/50">{item.mediaType}{item.year ? ` · ${item.year}` : ''}</span><span className="mt-1 block text-meta font-semibold text-accent/65">{item.mediaType === 'movie' ? item.watchedCount > 1 ? `${item.watchedCount} plays` : 'Watched' : `${item.watchedCount} episodes watched`}{item.runtimeMinutes ? ` · ${formatMinutes(item.runtimeMinutes)} estimated` : ''}</span><span className="mt-2 flex flex-wrap gap-1">{item.sources.map((source) => <span key={source} className={`rounded-md border px-1.5 py-0.5 text-tag font-bold ${sourceColors[source]}`}>{sourceLabels[source]}</span>)}</span></span>
      {item.watchedAt && <span className="self-start whitespace-nowrap text-tag text-white/20">{new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric', year: '2-digit' }).format(new Date(item.watchedAt))}</span>}
    </button>)}</div>}
    {filtered.length > 0 && <div className="flex flex-col items-center gap-3 py-3"><p className="text-meta text-white/25">Showing {Math.min(visibleCount, filtered.length)} of {filtered.length} deduplicated titles</p>{visibleCount < filtered.length && <button onClick={() => setVisibleCount((count) => count + PAGE_SIZE)} className="rounded-xl border border-accent/20 bg-accent/10 px-6 py-2.5 text-xs font-bold text-accent transition-all hover:bg-accent/20">Load {Math.min(PAGE_SIZE, filtered.length - visibleCount)} more</button>}</div>}
    {!loading && selected.length > 0 && filtered.length === 0 && <div className="py-16 text-center text-sm text-white/50">No history matches these filters.</div>}
  </section>
}
