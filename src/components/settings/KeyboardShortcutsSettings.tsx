  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-accent/15 bg-accent/[0.06] px-5 py-4">
        ...
        <p className="mt-1 text-xs leading-relaxed text-white/60">They are paused while you type in an input, text area, or editable field.</p>
      </div>
            ...
            {section.shortcuts.map((shortcut) => (
              <div key={shortcut.label} className="flex items-center justify-between gap-6 px-6 py-4">
                <div className="min-w-0">
                  ...
                  {shortcut.description && <p className="mt-0.5 text-xs leading-relaxed text-white/60">{shortcut.description}</p>}
                </div>
